...
def score_unscored_tenders(
    ...
    for the current model version and ``project_id``.

    Scores are clamped to [1.0, 5.0] because 0 means "ambiguous / manual review".

    Returns the number of rows written to ``tender_model_scores``.
    """
    ...
    # Find tender_ids already scored for this project + version
    already_resp = (
    ...
    logger.info("Already scored: %d tenders", len(already_scored))

    # Fetch all tenders in batches
    ...
    while True:
        ...
        if to_score:
            ...
            logger.info("Scored batch of %d (total so far: %d)", len(upsert_rows), total_written)
